    ...
    playCard(card, cardElement) {
        ...
        if (!topCard || card.suit === topCard.suit || card.value === topCard.value) {
            ...
            if (this.hand.length === 0) {
                alert(this.isAI ? "AI vyhrálo!" : "Gratulujeme! Vyhrál jsi!");
                location.reload();
            } else {
                game.endTurn();
            }
        } else {
            alert("Karta nesouhlasí barvou nebo hodnotou!");
        }
    }
}
...
class Game {
    ...
    updateTurnIndicator() {
        document.body.style.backgroundColor = this.isPlayerTurn ? '#2e8b57' : '#8b0000';
    }
}
